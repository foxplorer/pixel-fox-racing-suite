...
import { startFinishPosition, startFinishDirection } from './TrackData'
...
// Benches and stairs always visible, only foxes hide at distance
...
export const StadiumSeating: React.FC<StadiumSeatingProps> = ({
  ...
  getHeightAtPosition,
  foxDensityScale = 1,
  showFlatFoundation = false
}) => {
  const foxGroupRef = useRef<THREE.Group>(null)
  const { camera } = useThree()

  // Distance-based visibility for foxes only (benches/stairs always visible)
  const basePosition = customPosition || startFinishPosition
  useFrame(() => {
    const shouldShowFoxes = shouldRenderStadiumDetail(camera.position, basePosition)

    if (foxGroupRef.current && foxGroupRef.current.visible !== shouldShowFoxes) {
      foxGroupRef.current.visible = shouldShowFoxes
    }
  })
  ...
  const stadiumData = useMemo(() => {
    ...
    const placement = getStadiumStandPlacement({
      ...
      groundY: basePosition.y - 1
    })
    if (getHeightAtPosition) {
      placement.leftPos.y = getHeightAtPosition(placement.leftPos.x, placement.leftPos.z) - 0.5
      placement.rightPos.y = getHeightAtPosition(placement.rightPos.x, placement.rightPos.z) - 0.5
    }
    return placement
  }, [customPosition, customDirection, distanceFromTrackProp, getHeightAtPosition])
  ...
  const createFoundation = (position: THREE.Vector3, rotation: number, sideKey: string) => {
    const baseWidth = seatsPerRow * seatWidth + STAIR_COVER_WIDTH * 2 + 12
    const baseDepth = rows * rowDepth + 14
    const grassWidth = baseWidth + 42
    const grassDepth = baseDepth + 38

    return (
      <group key={`foundation-${sideKey}`} position={[position.x, position.y + 0.05, position.z]} rotation={[0, rotation, 0]}>
        <mesh position={[0, 0, baseDepth / 2 - 2]} receiveShadow>
          <boxGeometry args={[grassWidth, 0.44, grassDepth]} />
          <meshStandardMaterial color="#3f7651" roughness={0.96} />
        </mesh>
        <mesh position={[0, 0.2, baseDepth / 2 - 2]} receiveShadow>
          <boxGeometry args={[baseWidth, 0.4, baseDepth]} />
          <meshStandardMaterial color="#4a4a4a" roughness={0.92} />
        </mesh>
        <mesh position={[0, 0.42, baseDepth / 2 - 2]} receiveShadow>
          <boxGeometry args={[baseWidth - 3, 0.04, baseDepth - 3]} />
          <meshStandardMaterial color="#5a5a5a" roughness={0.88} />
        </mesh>
      </group>
    )
  }
  ...
  return (
    <group>
      {showFlatFoundation && (side === 'left' || side === 'both') && createFoundation(stadiumData.leftPos, stadiumData.leftRotation, 'left')}
      {showFlatFoundation && (side === 'right' || side === 'both') && createFoundation(stadiumData.rightPos, stadiumData.rightRotation, 'right')}

      {/* Roofs - ALWAYS visible (simple geometry, good landmark) */}
      ...
      {/* Stands with benches and stairs - ALWAYS visible */}
      ...
      {/* Foxes - only visible within 1000 units */}
