...
export const StadiumSeating: React.FC<StadiumSeatingProps> = ({
  ...
  useFrame(() => {
    const shouldShowFoxes = shouldRenderStadiumDetail(camera.position, startFinishPosition)
  ...
  const stadiumData = useMemo(() => {
    return getStadiumStandPlacement({
      basePosition: startFinishPosition,
      baseDirection: startFinishDirection,
      distanceFromTrack: 38,
      groundY: -0.5 // San Luis track ground level (raised from -1)
    })
  }, [])
  ...
  return (
    <group>
      ...
      {createRoof(stadiumData.leftPos, stadiumData.leftRotation)}
      {createRoof(stadiumData.rightPos, stadiumData.rightRotation)}

      {/* Stands with benches and stairs - ALWAYS visible */}
      {createStand(stadiumData.leftPos, stadiumData.leftRotation, 0)}
      {createStand(stadiumData.rightPos, stadiumData.rightRotation, 1)}
      ...
      <group ref={foxGroupRef}>
        {foxPlacements.length > 0 && textureAtlas && (
          <BillboardStadiumFoxes
            ...
            atlasSize={atlasSize}
            stadiumPosition={startFinishPosition}
