...
export const BarGraphMouth = ({ todisplay }: BarGraphProps) => {
        ...
        for (let i = 0; i < foxlength; i++) {
          if (kkk[i] !== undefined) if (kkk[i].origin.data.map.subTypeData.collectionId === "1611d956f397caa80b56bc148b4bce87b54f39b234aeca4668b4d5a7785eb9fa_0") {
              ...
              if (traits && traits[3] && traits[3].value) {
                if (bginfo.length === 0){
                  let feed = {name: traits[3].value, amount: 1};
                  bginfo.push(feed);
                }
                else{
                ...
                for(let j = 0; j < blength; j++){
                  if (bginfo[j].name === traits[3].value){
...
// for (let p = 0; p < jjj; i++) {
//   if (kkk[i] !== undefined) if(temparray[p].name === kkk[i].origin.data.map.subTypeData.traits[3].value){
...
//     if (bgOptions.includes(tempstring[i].origin.data.map.subTypeData.traits[3].value)) {
//       //do nothing
//     } else {
//       bgOptions.push(tempstring[i].origin.data.map.subTypeData.traits[3].value);
//       //get bgcolors for chart
//       temparray[bgcolorcount] = {name: tempstring[i].origin.data.map.subTypeData.traits[3].value, id: undefined};
//       bgcolorcount ++;
