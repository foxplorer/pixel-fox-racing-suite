...
import { SNOW_TREE_PALETTE, TreeInstances } from '../../racing/components/TreeInstances'
...
export const SimpleTrees: React.FC<SimpleTreesProps> = ({
  ...
  const trees = useMemo(() => {
    ...
    // Notify parent of tree positions for collision detection
    if (onTreesGenerated) {
      onTreesGenerated(treePositions)
    }
    
    return treePositions
  }, [count, area, trackCurve, onTreesGenerated, advertisingBoards])

  return <TreeInstances trees={trees} palette={SNOW_TREE_PALETTE} castShadow={false} receiveShadow={false} />
}
