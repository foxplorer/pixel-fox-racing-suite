...
const CurvedBoard: React.FC<CurvedBoardData> = React.memo(({ curve, startT, endT, height, offset, side }) => {
  ...
  const curvePoints = useMemo(() => {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      // Sharp corner smoothing: if perpendicular direction changed too much, blend with previous
      if (prevPerpDir) {
        const dot = perpDir.dot(prevPerpDir)
        if (dot < 0.7) { // Sharp corner detected (more than ~45 degree change)
          // Blend 70% previous, 30% current for smoother transition
